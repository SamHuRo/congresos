...
%% 0.1mW
       ...
       hold on
       plot3(dat(261, : ,1), (Phi2ps(1,26)+0.1)*ones(1,length(dat(i,:,1))), (10^6.*spectralin(261, : ,1)))
       view(0,70)
...
%% 1mW  
       ...
       hold on
       plot3(dat(261, : ,1), (Phi2ps(1,26)+0.1)*ones(1,length(dat(i,:,1))), (10^7.*spectralin(261, : ,1)))
       view(0,70)
       view(0,70)
...
% hold on
% plot(Phi2ps,PoutnW(261:286,1),'LineWidth',2);
plot(Phi2ps,PoutuW(235:260,1)/13,'LineWidth',2);
hold on
plot(Phi2ps,PoutuW(209:234,1)/11,'LineWidth',2);
plot(Phi2ps,PoutuW(183:208,1)/9,'LineWidth',2);
plot(Phi2ps,PoutuW(157:182,1)/7,'LineWidth',2);
plot(Phi2ps,PoutuW(131:156,1)/6,'LineWidth',2);
plot(Phi2ps,PoutuW(105:130,1)/5,'LineWidth',2);
plot(Phi2ps,PoutuW(79:104,1)/3,'LineWidth',2);
plot(Phi2ps,PoutuW(53:78,1)/2,'LineWidth',2);
plot(Phi2ps,PoutuW(27:52,1)/1,'LineWidth',2);
plot(Phi2ps,PoutuW(1:26,1)/0.1,'LineWidth',2);
xlim([-3 3])
box on
grid on
set(gca,'FontSize',12,'linewidth',2)
xlabel('$\phi^{(2)} (\rm ps^{2})$','fontsize',14,'interpreter', 'latex')
ylabel('(Pout/Pin)*$10^6$','fontsize',14, 'interpreter', 'latex')
%legend('P_{in}=10 mW','P_{in}=7 mW','P_{in}=2.5 mW','P_{in}=1 mW','P_{in}=0.1 mW','Location','NorthEastOutside')
%%
PoutoPinphiminus=[PoutuW(1,1)/0.1 PoutuW(27,1)/1 PoutuW(53,1)/2 PoutuW(79,1)/3 PoutuW(105,1)/5 PoutuW(131,1)/6 PoutuW(157,1)/7 PoutuW(183,1)/9 PoutuW(209,1)/11 PoutuW(235,1)/13];
PoutoPinphizero=[PoutuW(13,1)/0.1 PoutuW(39,1)/1 PoutuW(65,1)/2 PoutuW(91,1)/3 PoutuW(117,1)/5 PoutuW(143,1)/6 PoutuW(169,1)/7 PoutuW(195,1)/9 PoutuW(221,1)/11 PoutuW(247,1)/13];
PoutoPinphiplus=[PoutuW(26,1)/0.1 PoutuW(52,1)/1 PoutuW(78,1)/2 PoutuW(104,1)/3 PoutuW(130,1)/5 PoutuW(156,1)/6 PoutuW(182,1)/7 PoutuW(208,1)/9 PoutuW(234,1)/11 PoutuW(260,1)/13];
Pin=[0.1 1 2 3 5 6 7 9 11 13];
figure
plot(Pin, PoutoPinphiminus,'LineWidth',2)
hold on
plot(Pin, PoutoPinphizero,'LineWidth',2)
plot(Pin, PoutoPinphiplus,'LineWidth',2)
box on
grid on
set(gca,'FontSize',12,'linewidth',2)
xlabel('Pin [mW]','fontsize',14,'interpreter', 'latex')
ylabel('(Pout/Pin)*$10^6$','fontsize',14, 'interpreter', 'latex')
%%
...
%%    
spectralin5mW=spectralin(105:130,:);
surfpower(Y,X,spectralin5mW'.*10^6)
%%    
spectralin6mW=spectralin(131:156,:);
surfpower(Y,X,spectralin6mW'.*10^6)
%%    
spectralin7mW=spectralin(157:182,:);
surfpower(Y,X,spectralin7mW'.*10^6)
%%    
spectralin9mW=spectralin(183:208,:);
surfpower(Y,X,spectralin9mW'.*10^6)
%%    
spectralin11mW=spectralin(209:234,:);
surfpower(Y,X,spectralin11mW'.*10^6)
%%    
spectralin13mW=spectralin(235:260,:);
surfpower(Y,X,spectralin13mW'.*10^6)
%%    
