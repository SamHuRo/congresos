...
%        bari(i)=sum(dlambda.*abs(s))/sum(abs(s));
...
for k=0:25
    ...
    Phi2ps(1,k+1)=0.238.*(Position2(1,k+1))-2.73+0.1;
end
...
%% 0.1mW
       ...
       hold on
       plot3(dat(183, : ,1), (Phi2ps(1,26)+0.1)*ones(1,length(dat(i,:,1))), (10^6.*spectralin(183, : ,1)))
       view(0,70)
...
%% 1mW  
       ...
       hold on
       plot3(dat(183, : ,1), (Phi2ps(1,26)+0.1)*ones(1,length(dat(i,:,1))), (10^7.*spectralin(183, : ,1)))
       view(0,70)
       view(0,70)
...
plot(Phi2ps,PoutuW(157:182,1)/9,'LineWidth',2);
hold on
plot(Phi2ps,PoutuW(131:156,1)/7,'LineWidth',2);
plot(Phi2ps,PoutuW(105:130,1)/6,'LineWidth',2);
plot(Phi2ps,PoutuW(79:104,1)/5,'LineWidth',2);
plot(Phi2ps,PoutuW(53:78,1)/3,'LineWidth',2);
...
%legend('P_{in}=10 mW','P_{in}=7 mW','P_{in}=2.5 mW','P_{in}=1 mW','P_{in}=0.1 mW','Location','NorthEastOutside')
%%
PoutoPinphiminus=[PoutuW(1,1)/0.1 PoutuW(27,1)/1 PoutuW(53,1)/3 PoutuW(79,1)/5 PoutuW(105,1)/6 PoutuW(131,1)/7 PoutuW(157,1)/9];
PoutoPinphizero=[PoutuW(13,1)/0.1 PoutuW(39,1)/1 PoutuW(65,1)/3 PoutuW(91,1)/5 PoutuW(117,1)/6 PoutuW(143,1)/7 PoutuW(169,1)/9];
PoutoPinphiplus=[PoutuW(26,1)/0.1 PoutuW(52,1)/1 PoutuW(78,1)/3 PoutuW(104,1)/5 PoutuW(130,1)/6 PoutuW(156,1)/7 PoutuW(182,1)/9];
Pin=[0.1 1 3 5 6 7 9];
figure
...
%%    
spectralin3mW=spectralin(53:78,:);
surfpower(Y,X,spectralin3mW'.*10^6)
%%    
spectralin5mW=spectralin(79:104,:);
surfpower(Y,X,spectralin5mW'.*10^6)
%%    
spectralin6mW=spectralin(105:130,:);
surfpower(Y,X,spectralin6mW'.*10^6)
%%    
spectralin7mW=spectralin(131:156,:);
surfpower(Y,X,spectralin7mW'.*10^6)
%%    
spectralin9mW=spectralin(157:182,:);
