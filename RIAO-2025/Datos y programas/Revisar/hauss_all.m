...
beta    = .8e-2/1e9;             %10e-2/1e9;
sigmaN  = -1.35e-21*1e-6;        %-7e-21*1e-6;           
sigmaA  = 1.45e-17*1e-4;         %3.7e-18*1e-4;
...
Sin   = sqrt(P0)*(exp(-(t-Tmax/2).^2/(2*T0^2))).*exp(1i*circshift(cphia',-110000)');
...
%                     - gamma*a(k) + sqrt(2/taue)*Sin(k);
    ...
    fFCA = sigmaA*Np(k)*L/(2*tauR);
        ...
        a_dot(k) =  1i*dw*a(k) ... 
                    - 1*1i*wRes*n2*abs(a(k)).^2/tauR/Seff*a(k)/n0 ...
                    - 0*1i*wRes*sigmaN*Np(k)*a(k)/n0 ...
                    - 0*fTPA*a(k) ...
                    - 0*fFCA*a(k) ...
                    - gamma*a(k) ...
                    + sqrt(2/taue)*Sin(k);
    ...
    Dw = -w0*( 1*sigmaN*Np/n0 + n2*abs(a).^2/tauR/Seff/n0);
...
plot(...
        ... %gamma*t, abs(Sin).^2, ...
    gamma*t-gamma*Tmax/2, abs(a).^2/tauR/P0); %, gamma*t, abs(Sout).^2);
% 
% figure(2)
% 
...
axis([-6 6 0 max(abs(a).^2/tauR/P0)]);
